...
The OFLAT platform is the graphical interface of the project, available online to the public. Its main goal is to serve as an easy-to-use educational tool. It features a static menu on the left sidebar and a dynamic workspace on the right, which changes according to the options selected on the screen. In Figure 4.1, you can see the main screen when you click on the “New model -> Finite automata” option in the sidebar.

The platform is intuitive and user-friendly. For example, to create a finite automaton, you first need to choose the option in the sidebar and then right-click on the blank workspace, where the option to create a state appears. The state can be either final or initial (Figure 4.2), and an initial state is already displayed by default. By right-clicking on the state, you can add transitions (Figure 4.3) and thus build a finite automaton. In the lower-left corner of the white screen, information about the automaton is displayed, such as whether it is deterministic, whether it is minimal, the number of useless states, the number of states, and transitions. In Figure 4.4, we can see that the created automaton is deterministic and not minimal. We can then click on the “minimize automaton” option to view a minimal version (Figure 4.5).
