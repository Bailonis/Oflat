...
\section{Tests: Trees and Attribute Annotations}
This chapter includes every test defined in the test module, with a brief purpose statement and a corresponding figure. 
...
\subsection{\texttt{test\_has\_cycles\_ok} (\texttt{ok\_ag})}
 Acyclic grammar \(\;S\to F\{v(S)=v(F)\},\ F\to a\{v(F)=1\}\;\). No dependency cycle; a concrete tree for the terminal word \texttt{a} is shown. (The test itself only checks the cycle predicate.) 
